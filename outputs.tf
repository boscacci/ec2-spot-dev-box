output "instance_id" {
  ...
  value       = var.enable_instance ? aws_instance.dev_box[0].id : ""
}

output "public_ip" {
  description = "Public IP of the dev box (SSH target)"
  value       = var.enable_eip ? aws_eip.dev_box[0].public_ip : (var.enable_instance ? aws_instance.dev_box[0].public_ip : "")
}
...
output "ssh_command" {
  ...
  value       = var.enable_eip ? "ssh -A -i ~/.ssh/${var.key_name}.pem ec2-user@${aws_eip.dev_box[0].public_ip}" : (var.enable_instance ? "ssh -A -i ~/.ssh/${var.key_name}.pem ec2-user@${aws_instance.dev_box[0].public_ip}" : "")
}
...
output "spot_price" {
  description = "Spot max bid price (empty = up to on-demand)"
